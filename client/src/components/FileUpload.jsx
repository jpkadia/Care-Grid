...
const FileUpload = ({ label, hint, multiple = false, compact = false, required = false, onChange, onDiscard, files, icon = 'fa-cloud-arrow-up' }) => {
  ...
  return (
    <div className={compact ? 'w-full max-w-sm' : 'w-full'}>
      <label className="block cursor-pointer group">
        <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" multiple={multiple} required={required && !selectedFiles.length} onChange={onChange} className="sr-only" />
        {previews.length ? (
