...
const Home = () => {
  ...
  const validateRegistration = () => {
    ...
    if (!profilePhoto) {
      toast('Please select a profile photo.', 'error');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('/api/auth/registration-otp', { email: formData.email });
        ...
        <div className="p-5 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input type="text" name="name" minLength="2" maxLength="100" placeholder="Enter your Name" required onChange={handleInputChange} className={inputClass} />
            </div>

            <div>
              <label className={labelClass}><i className={`fas fa-graduation-cap ${iconClass}`}></i> Qualification</label>
              <input type="text" name="education" placeholder="Enter your Degree" required onChange={handleInputChange} className={inputClass} />
            </div>
            ...
            <div>
              ...
              <input type="text" name="clinicName" placeholder="Enter Clinic Name" required onChange={handleInputChange} className={inputClass} />
            </div>

            <div>
              <label className={labelClass}><i className={`fas fa-map-marker-alt ${iconClass}`}></i> Clinic Address</label>
              <textarea name="location" rows="3" placeholder="Enter Full Address" required onChange={handleInputChange} className={`${inputClass} resize-y`}></textarea>
            </div>

            <div>
              <label className={labelClass}><i className={`fas fa-phone ${iconClass}`}></i> Phone Number</label>
              <input type="tel" name="phone" inputMode="numeric" autoComplete="tel" pattern="\d{10}" minLength="10" maxLength="10" title="Enter exactly 10 digits" placeholder="Enter 10-digit Mobile Number" required onChange={(e) => { e.target.value = e.target.value.replace(/\D/g, '').slice(0, 10); handleInputChange(e); }} className={inputClass} />
            </div>

            <div>
              <label className={labelClass}><i className={`fas fa-envelope ${iconClass}`}></i> Email Address</label>
              <input type="email" name="email" placeholder="Enter Email" required onChange={handleInputChange} className={inputClass} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200 mt-6">
                <div>
                  ...
                  <input type="password" name="password" minLength="8" autoComplete="new-password" placeholder="Create Password" required onChange={handleInputChange} className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}><i className={`fas fa-lock ${iconClass}`}></i> Confirm Password</label>
                  <input type="password" name="confirmPassword" minLength="8" autoComplete="new-password" placeholder="Confirm Password" required onChange={handleInputChange} className={inputClass} />
                </div>
            </div>
            
            {passwordError && (
              <div className="text-red-500 text-sm font-semibold mt-2">{passwordError}</div>
            )}
            ...
            <div className="pt-4 border-t border-slate-200">
              ...
              <div className="flex justify-center">
                <FileUpload label="Upload profile photo" hint="JPG, PNG or WebP, maximum 5MB" compact required onChange={handleProfilePhoto} onDiscard={() => setProfilePhoto(null)} files={profilePhoto} icon="fa-user-doctor" />
              </div>
            </div>

            <div>
              <label className={labelClass}><i className={`fas fa-images ${iconClass}`}></i> Clinic Photos (Max 8)</label>
              <FileUpload label="Upload clinic gallery" hint="Up to 8 JPG, PNG or WebP images, maximum 5MB each" multiple onChange={handleSliderImages} onDiscard={(index) => setSliderImages(current => current.filter((_, itemIndex) => itemIndex !== index))} files={sliderImages} icon="fa-images" />
            </div>
